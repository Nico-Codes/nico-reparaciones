...
@section('title', 'Admin - Catalogo de dispositivos')
...
<div class="store-shell space-y-6">
  <div class="reveal-item rounded-3xl border border-zinc-200/80 bg-gradient-to-r from-white via-sky-50/60 to-white p-4 sm:p-6">
    <div class="page-head mb-0">
      <div>
        <div class="page-title">Catalogo de dispositivos</div>
        <div class="page-subtitle">Gestiona marcas, modelos y fallas por tipo. En lugar de borrar, desactiva.</div>
      </div>
  ...
  <div class="reveal-item card">
    <div class="card-head">
      <div>
        <div class="font-black">Filtro de catalogo</div>
        <div class="text-xs text-zinc-500">Selecciona tipo y marca para administrar cada bloque.</div>
      </div>
    </div>
    ...
    <div class="card-body">
      <form method="GET" class="grid gap-3 md:grid-cols-3 items-end">
        ...
        <div class="text-xs text-zinc-500">
          Tip: en "Nueva reparacion" solo se muestran items activos.
        </div>
      </form>
    </div>
  </div>

  <div class="reveal-item grid gap-4 lg:grid-cols-3">
    <div class="card">
      <div class="card-head">
      ...
      <div class="card-body">
        ...
        <div class="mt-4">
          @if(!$typeId)
            <p class="text-sm text-zinc-600">Elegi un tipo para ver o crear marcas.</p>
              ...
              @foreach($brands as $b)
                <div class="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm">
                  ...
                  <form method="POST" action="{{ route('admin.deviceCatalog.brands.toggle', $b) }}" class="mt-2">
                    @csrf
                    <button class="btn-ghost h-10 w-full justify-center">{{ $b->active ? 'Desactivar' : 'Activar' }}</button>
                  </form>
                </div>
              @endforeach
            </div>
                ...
                <tbody class="divide-y divide-zinc-100">
                  @foreach($brands as $b)
                    <tr>
                      ...
                      <td class="py-2">@if($b->active)<span class="badge badge-emerald">Activa</span>@else<span class="badge badge-zinc">Inactiva</span>@endif</td>
                      <td class="py-2 text-right">
                        <form method="POST" action="{{ route('admin.deviceCatalog.brands.toggle', $b) }}">@csrf
                          <button class="btn-ghost btn-sm h-10">{{ $b->active ? 'Desactivar' : 'Activar' }}</button>
                        </form>
                      </td>
                    </tr>
                  @endforeach
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="mt-4">
          @if(!$typeId)
            <p class="text-sm text-zinc-600">Elegi un tipo primero.</p>
          @elseif(!$brandId)
            <p class="text-sm text-zinc-600">Elegi una marca para ver o crear modelos.</p>
              ...
              @foreach($models as $m)
                <div class="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm">
                  <div class="flex items-start justify-between gap-2">
                    ...
                    @if($m->active)<span class="badge badge-emerald">Activo</span>@else<span class="badge badge-zinc">Inactivo</span>@endif
                  </div>
                  ...
                  <form method="POST" action="{{ route('admin.deviceCatalog.models.toggle', $m) }}" class="mt-2">@csrf
                    <button class="btn-ghost h-10 w-full justify-center">{{ $m->active ? 'Desactivar' : 'Activar' }}</button>
                  </form>
                </div>
              @endforeach
            </div>
                ...
                <tbody class="divide-y divide-zinc-100">
                  @foreach($models as $m)
                    <tr>
                      <td class="py-2">
                        <div class="font-medium text-zinc-900">{{ $m->name }}</div>
                        <div class="text-xs text-zinc-500">Grupo: {{ $m->group?->name ?? '-' }}</div>
                        <details class="mt-1">
                          <summary class="text-xs text-sky-700 cursor-pointer select-none">Renombrar</summary>
                      ...
                      <td class="py-2">@if($m->active)<span class="badge badge-emerald">Activo</span>@else<span class="badge badge-zinc">Inactivo</span>@endif</td>
                      <td class="py-2 text-right">
                        <form method="POST" action="{{ route('admin.deviceCatalog.models.toggle', $m) }}">@csrf
                          <button class="btn-ghost btn-sm h-10">{{ $m->active ? 'Desactivar' : 'Activar' }}</button>
                        </form>
                      </td>
                    </tr>
                  @endforeach
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="mt-4">
          @if(!$typeId)
            <p class="text-sm text-zinc-600">Elegi un tipo para ver o crear fallas.</p>
              ...
              @foreach($issues as $i)
                <div class="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm">
                  <div class="flex items-start justify-between gap-2">
                    ...
                    @if($i->active)<span class="badge badge-emerald">Activa</span>@else<span class="badge badge-zinc">Inactiva</span>@endif
                  </div>
                  ...
                  <form method="POST" action="{{ route('admin.deviceCatalog.issues.toggle', $i) }}" class="mt-2">@csrf
                    <button class="btn-ghost h-10 w-full justify-center">{{ $i->active ? 'Desactivar' : 'Activar' }}</button>
                  </form>
                </div>
              @endforeach
            </div>
                ...
                <tbody class="divide-y divide-zinc-100">
                  @foreach($issues as $i)
                    <tr>
                      ...
                      <td class="py-2">@if($i->active)<span class="badge badge-emerald">Activa</span>@else<span class="badge badge-zinc">Inactiva</span>@endif</td>
                      <td class="py-2 text-right">
                        <form method="POST" action="{{ route('admin.deviceCatalog.issues.toggle', $i) }}">@csrf
                          <button class="btn-ghost btn-sm h-10">{{ $i->active ? 'Desactivar' : 'Activar' }}</button>
                        </form>
                      </td>
                    </tr>
                  @endforeach
