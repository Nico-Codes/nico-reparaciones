...
<div class="store-shell space-y-6">
  ...
  @if(!empty($freshRecoveryCodes))
    <div class="reveal-item rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900 space-y-3">
      ...
      <div class="text-xs text-amber-800">Disponibles por {{ (int) $recoveryExportTtlMinutes }} minutos desde su generacion.</div>
      @if($recoveryExportToken)
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
          <a class="btn-outline h-11 w-full justify-center sm:w-auto" href="{{ route('admin.two_factor.recovery.download', ['token' => $recoveryExportToken]) }}">Descargar .txt</a>
          <a class="btn-outline h-11 w-full justify-center sm:w-auto" href="{{ route('admin.two_factor.recovery.print', ['token' => $recoveryExportToken]) }}" target="_blank" rel="noopener">Imprimir / Guardar PDF</a>
  ...
  <div class="reveal-item card">
    ...
    <div class="card-body grid gap-4">
      ...
      @else
        <div class="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Tu panel admin aun no tiene doble factor. Activalo para bloquear accesos aunque filtren tu contrasena.</div>
        ...
        @else
          <div class="grid gap-3">
            ...
            <div class="text-xs text-zinc-500">Apps sugeridas: Google Authenticator, Authy, Microsoft Authenticator.</div>
          </div>
