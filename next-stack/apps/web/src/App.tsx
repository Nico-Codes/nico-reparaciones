      ...
      <main className="mx-auto max-w-6xl px-4 py-10">
        <section className="overflow-hidden rounded-3xl border border-zinc-200 bg-gradient-to-br from-sky-700 via-sky-600 to-cyan-500 p-8 text-white shadow-xl">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/10 px-3 py-1 text-xs font-bold">
              ...
              Migración total en progreso
            </div>
            <h1 className="mt-4 text-3xl font-black tracking-tight md:text-5xl">
              Nuevo stack: React + NestJS + Prisma + PostgreSQL
            </h1>
            <p className="mt-3 text-sm text-sky-50 md:text-base">
              Base inicial del proyecto nueva creada en paralelo para migrar módulos sin perder trazabilidad.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Button asChild>
                <Link to="/store">Ver Tienda Next</Link>
              </Button>
              <Button variant="secondary" asChild>
                <Link to="/admin">Ver Admin Next</Link>
              </Button>
              <Button variant="outline" asChild>
                <Link to="/repairs">Mis reparaciones</Link>
              </Button>
              <Button variant="outline" asChild>
                <Link to="/auth/login">Probar Auth</Link>
              </Button>
              <Button variant="outline" asChild>
                <Link to="/auth/bootstrap-admin">Bootstrap Admin</Link>
              </Button>
              <Button variant="outline" asChild>
                <Link to="/cart">Ver carrito{cartCount ? ` (${cartCount})` : ''}</Link>
              </Button>
            </div>
          </div>
        </section>
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-sm text-zinc-600">Catálogo, filtros, carrito y checkout migrables a SPA con API REST.</p>
          </div>
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-sm text-zinc-600">Flujo principal del negocio. Se migrará por módulos con tests y contratos.</p>
          </div>
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
            <div className="mb-2 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-violet-50 text-violet-700">
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-zinc-600">Vista placeholder para iniciar migración funcional por módulos.</p>
        <div className="mt-4">
...
export default function App() {
  return (
    <Routes>
      ...
      <Route path="/admin" element={<RequireAdmin><Placeholder title="Admin Next (React)" /></RequireAdmin>} />
    </Routes>
  );
}
