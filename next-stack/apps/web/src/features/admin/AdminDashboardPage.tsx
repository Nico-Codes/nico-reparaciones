...
export function AdminDashboardPage() {
  ...
  const cards = useMemo(() => {
    if (!data) return [];
    return [
      { label: 'Pedidos pendientes', value: data.metrics.orders.pendingFlow, icon: ClipboardList, tone: 'sky' as const },
      { label: 'Reparaciones abiertas', value: data.metrics.repairs.open, icon: Wrench, tone: 'emerald' as const },
      { label: 'Stock bajo', value: data.metrics.products.lowStock, icon: Boxes, tone: 'amber' as const },
      { label: 'Sin stock', value: data.metrics.products.outOfStock, icon: AlertTriangle, tone: 'rose' as const },
    ];
  }, [data]);
    ...
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600">Resumen operativo y accesos a modulos principales.</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {loading || !data
          ? Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-28 animate-pulse rounded-2xl border border-zinc-200 bg-white" />)
          : cards.map((card) => <MetricCard key={card.label} {...card} />)}
      </div>

      <div className="mt-4 grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
          <h2 className="mb-3 text-sm font-bold uppercase tracking-wide text-zinc-500">Accesos rapidos</h2>
          <div className="grid gap-2 sm:grid-cols-2">
            <QuickLink to="/admin/orders" title="Pedidos" desc="Ver pedidos y cambiar estados" />
            <QuickLink to="/admin/repairs" title="Reparaciones" desc="Alta y seguimiento" />
            <QuickLink to="/admin/products" title="Productos" desc="Stock, precio, activos" />
            <QuickLink to="/admin/users" title="Usuarios" desc="Roles y cuentas" />
            <QuickLink to="/admin/device-catalog" title="Catalogo dispositivos" desc="Marcas, modelos y fallas" />
            <QuickLink to="/admin/settings" title="Configuracion" desc="Negocio, correo y ajustes" />
          </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        <section className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
          <h2 className="text-sm font-bold uppercase tracking-wide text-zinc-500">Ultimos pedidos</h2>
          <div className="mt-3 space-y-2">
            {loading || !data ? (
              <div className="rounded-xl border border-zinc-200 p-3 text-sm">Cargando...</div>
            ) : data.recent.orders.length === 0 ? (
              <div className="rounded-xl border border-zinc-200 p-3 text-sm">Sin pedidos todavia.</div>
            ) : (
        ...
        <section className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
          <h2 className="text-sm font-bold uppercase tracking-wide text-zinc-500">Ultimas reparaciones</h2>
          <div className="mt-3 space-y-2">
            {loading || !data ? (
              <div className="rounded-xl border border-zinc-200 p-3 text-sm">Cargando...</div>
            ) : data.recent.repairs.length === 0 ? (
              <div className="rounded-xl border border-zinc-200 p-3 text-sm">Sin reparaciones todavia.</div>
            ) : (
