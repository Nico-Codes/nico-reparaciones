...
import {
  ...
  REPAIR_TEMPLATE_ORDER,
  WHATSAPP_VARIABLES,
} from './admin-whatsapp.helpers';

type AdminWhatsappFeedbackProps = {
  error: string;
  success: string;
};

type AdminWhatsappTemplatesSectionProps = {
  loading: boolean;
  saving: boolean;
...
type AdminWhatsappActionsProps = {
  loading: boolean;
  saving: boolean;
  onReset: () => void;
  onSave: () => void;
...
type AdminWhatsappLogsSectionProps = {
  logsLoading: boolean;
  logs: WhatsappLogItem[];
  onRefresh: () => void;
};

export function AdminWhatsappHero() {
  return (
...
export function AdminWhatsappFeedback({ error, success }: AdminWhatsappFeedbackProps) {
  return (
    <>
      {error ? (
...
export function AdminWhatsappVariablesSection() {
  return (
...
export function AdminWhatsappTemplatesSection({
  loading,
  saving,
  templates,
  onChange,
}: AdminWhatsappTemplatesSectionProps) {
  return (
          ...
          <section key={template.key} className="card overflow-hidden">
            <div className="card-body !p-0">
              ...
              <div className="px-4 py-4">
                <textarea
                  rows={getWhatsappTemplateRows(template.key)}
...
export function AdminWhatsappActions({ loading, saving, onReset, onSave }: AdminWhatsappActionsProps) {
  return (
...
export function AdminWhatsappLogsSection({ logsLoading, logs, onRefresh }: AdminWhatsappLogsSectionProps) {
  return (
    <section className="card overflow-hidden">
      <div className="card-body !p-0">
                ...
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-zinc-500">
                      {log.targetType || 'repair'} · {log.targetId || '-'} ·{' '}
