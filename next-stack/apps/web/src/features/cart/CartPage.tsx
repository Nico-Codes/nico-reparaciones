    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight">Carrito</h1>
            <div className="text-sm text-zinc-600">Carrito local con validacion real contra API</div>
          </div>
        </div>
      ...
      {message ? (
        <div className="mb-4 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-900">
          {message}
        </div>
      ) : null}
      ...
      {!cart.items.length ? (
        <div className="rounded-2xl border border-zinc-200 bg-white p-8 text-center shadow-sm">
          <div className="text-lg font-black text-zinc-900">Tu carrito esta vacio</div>
          <div className="mt-2 text-sm text-zinc-600">Agrega productos desde la tienda para probar el flujo.</div>
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
          <div className="space-y-3">
            ...
            {(quote?.items ?? []).map((line) => (
              <div
                key={line.productId}
                className={`rounded-2xl border p-4 shadow-sm ${line.valid ? 'border-zinc-200 bg-white' : 'border-rose-200 bg-rose-50/60'}`}
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="font-black text-zinc-900">{line.name}</div>
                    <div className="mt-1 text-xs text-zinc-500">{line.category?.name ?? 'Sin categoria'}</div>
          ...
          <aside className="h-fit rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 space-y-2 text-sm">
              <div className="flex justify-between"><span>Items validos</span><span className="font-bold">{quote?.totals.itemsCount ?? 0}</span></div>
            ...
            <div className="mt-2 text-xs text-zinc-500">El checkout crea pedido real y descuenta stock.</div>
          </aside>
        </div>
      )}
    </div>
