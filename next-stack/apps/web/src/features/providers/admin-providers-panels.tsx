...
export function ProvidersStatsPanel({ summary }: { summary: ProvidersSummary }) {
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <ProviderStatCard title="INCIDENTES" value={String(summary.totalIncidents)} />
      <ProviderStatCard title="ABIERTOS" value={String(summary.openIncidents)} valueClass="text-amber-600" />
      <ProviderStatCard title="CERRADOS" value={String(summary.closedIncidents)} />
      <ProviderStatCard title="PERDIDA" value={money(summary.accumulatedLoss)} valueClass="text-rose-700" />
    </div>
  );
}

export function ProvidersPriorityPanel({
  ordered,
  savingOrder,
  onMovePriority,
  onSaveOrder,
...
export function ProvidersCreatePanel({
  draft,
  onDraftChange,
  onSave,
}: {
...
export function ProvidersTablePanel({
  loading,
  ordered,
  onPatchProvider,
  onSaveProvider,
...
function ProviderStatCard({ title, value, valueClass }: { title: string; value: string; valueClass?: string }) {
  return (
    <section className="card">
