...
describe('AdminProviderRegistryService', () => {
    ...
    expect(rows).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          searchConfigJson: expect.stringContaining('"items_path":"suggestions"'),
        }),
        expect.objectContaining({
          id: 'sup_custom',
          name: 'Proveedor Personalizado',
  ...
  it('deletes providers without special order profiles', async () => {
    ...
    await expect(service.deleteProvider('sup_test')).resolves.toEqual({ ok: true });
