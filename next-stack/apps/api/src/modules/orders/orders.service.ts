...
export class OrdersService {
  ...
  async checkout(input: CheckoutInput) {
    ...
    if (!validLines.length) {
      throw new BadRequestException('No hay items vÃ¡lidos para generar pedido');
    }

    // Si hay invÃ¡lidos, frenamos para evitar sorpresas.
    const invalid = quote.items.filter((i) => !i.valid);
    if (invalid.length > 0) {
      throw new BadRequestException({
        message: 'Hay items invÃ¡lidos en el carrito',
    ...
    const order = await this.prisma.$transaction(async (tx) => {
      // RevalidaciÃ³n de stock en transacciÃ³n antes de descontar.
      ...
      for (const line of validLines) {
        ...
        if (!p || !p.active) {
          throw new BadRequestException(`Producto invÃ¡lido en checkout: ${line.name}`);
        }
  ...
  private async createQuickSaleWhatsappLog(
    ...
    try {
      ...
      await this.whatsappService.createAndDispatchLog({
        ...
        message: `Venta rápida confirmada #${orderId}. Pago: ${paymentLabel}. Ítems: ${details.itemCount}.`,
        meta: {
