...
- Contexto: una vez resuelto el upload persistente de fondos de auth, el siguiente problema real quedo en la presentacion del panel visual. `AuthLayout` seguia aplicando un overlay oscuro fijo sobre la imagen y mantenia hardcodeados el eyebrow, el titulo, la descripcion y su color. Eso obligaba a tocar codigo para ajustes simples de branding y hacia que la imagen editable siguiera viendose tapada por una capa no deseada.
- Decision: mantener los fondos `desktop/mobile` dentro del circuito actual de branding, pero mover tambien el copy del panel visual a `appSetting` con cuatro claves nuevas: `auth_panel_eyebrow`, `auth_panel_title`, `auth_panel_description` y `auth_panel_text_color`. `StoreService.getBrandingAssets()` expone ese bloque en `/api/store/branding`, `AuthLayout` lo consume con defaults seguros y normalizacion de color HEX, y la pantalla `AdminVisualIdentityPage` agrega un bloque especifico para editar esos textos y el color desde configuracion. El overlay oscuro fijo se elimina del markup y del CSS, reemplazandolo por texto con sombra suave para no tapar la imagen.
- Impacto: el operador puede cambiar imagen, eyebrow, titulo, descripcion y color del panel visual de auth sin tocar codigo. El fondo deja de verse “ensuciado” por una capa negra fija y el branding del login/registro/reset queda controlado desde la misma vista de identidad visual.
- Alternativas consideradas: dejar el overlay fijo y solo exponer un toggle, o mover el copy de auth a otra pantalla de settings; descartado porque lo primero no resolvia la queja visual y lo segundo rompia la cohesión del subdominio de identidad visual.
- Archivos / modulos afectados: `next-stack/apps/api/src/modules/{admin/app-settings.registry.ts,store/store.service.ts,store/store.service.test.ts}`, `next-stack/apps/web/src/features/{auth/AuthLayout.tsx,admin/{AdminVisualIdentityPage.tsx,admin-visual-identity.helpers.ts,admin-visual-identity.helpers.test.ts,admin-visual-identity.sections.tsx},store/types.ts}`, `next-stack/apps/web/src/styles/auth.css`, `project-docs/{DECISIONS_LOG.md,architecture/ARCHITECTURE.md,frontend/FRONTEND_MAP.md}`, `CHANGELOG_AI.md`.
- Validacion requerida: `typecheck --workspace @nico/api`, `test --workspace @nico/api`, `build --workspace @nico/api`, `typecheck --workspace @nico/web`, `test --workspace @nico/web`, `build --workspace @nico/web`, `smoke:backend`, `smoke:web`, `git diff --check`.
- Responsable: Codex + operador humano
...
- Contexto: el backend ya generaba logs y disparos de WhatsApp al cambiar estados de reparacion, pero esa capa depende de configuracion de Meta Cloud y hoy envia texto libre, no templates oficiales. Eso dejaba un hueco operativo: el sistema sabia construir el mensaje correcto, pero si Cloud estaba apagado o incompleto el operador no tenia una salida ordenada desde la propia reparacion.
- Decision: mantener la logica actual de construccion de mensaje en `RepairsNotificationsService` como fuente unica, exponer un endpoint admin para obtener el borrador WhatsApp de una reparacion y otro para registrar la apertura manual, y agregar en el detalle admin un bloque `WhatsApp cliente` con preview del mensaje, telefono normalizado y CTA `Abrir WhatsApp`. Los logs distinguen `manual` vs `cloud` en `meta.deliveryMode`, sin agregar columna nueva ni migracion. La automatizacion real con Meta queda explicitamente postergada hasta migrar a templates oficiales.
- Impacto: la operacion diaria ya no depende de que Meta Cloud este listo para poder avisar al cliente. Cada reparacion puede abrir WhatsApp con el mensaje exacto para el estado guardado actual y dejar trazabilidad manual, mientras el backend conserva la base de Cloud/webhook para una etapa posterior.
- Alternativas consideradas: seguir esperando a Meta Cloud antes de habilitar cualquier UX de envio, o agregar una bandeja central de pendientes; descartado porque la primera bloquea valor inmediato y la segunda agrega una capa operativa innecesaria para esta etapa.
- Archivos / modulos afectados: `next-stack/apps/api/src/modules/{repairs/{repairs.controller.ts,repairs.service.ts,repairs-admin.service.ts,repairs-notifications.service.ts,repairs.helpers.ts,repairs.helpers.test.ts,repairs.types.ts},whatsapp/whatsapp.service.ts}`, `next-stack/apps/web/src/features/repairs/{api.ts,types.ts,use-admin-repair-detail.ts,admin-repair-detail-sidebar.tsx,AdminRepairDetailPage.tsx,admin-repair-detail.sections.tsx}`, `next-stack/apps/web/src/styles/repairs.css`, `project-docs/{DECISIONS_LOG.md,backend/BACKEND_MAP.md,frontend/FRONTEND_MAP.md}`, `CHANGELOG_AI.md`.
- Validacion requerida: `typecheck --workspace @nico/api`, `test --workspace @nico/api`, `build --workspace @nico/api`, `typecheck --workspace @nico/web`, `test --workspace @nico/web`, `build --workspace @nico/web`, `smoke:backend`, `smoke:web`, `git diff --check`.
- Responsable: Codex + operador humano
...
- Fecha: 2026-04-24
- Estado: aceptada
- Tema: el importador de listados por encargue selecciona por seccion/producto y puede recordar exclusiones por perfil
- Contexto: la primera version del importador de encargues resolvia parseo, preview, mapeo de categorias y apply, pero toda exclusion era por `rowId` de la corrida actual. Eso servia para conflictos puntuales, pero no para la necesidad operativa real: saltar marcas o tipos completos como `Motorola` y hacer que esa decision vuelva ya precargada en el siguiente listado del mismo proveedor/perfil. Ademas, la tabla plana de productos saturaba la vista cuando el operador solo queria decidir primero por seccion y luego, si hacia falta, por producto puntual.
- Decision: mantener el modelo actual del importador y agregar una capa de seleccion estable por perfil usando dos memorias separadas: `rememberedExcludedSectionKeysJson` y `rememberedExcludedSourceKeysJson` en `SpecialOrderImportProfile`. El preview/aplicacion acepta exclusiones explicitas por `sectionKey`, por `sourceKey` y, solo para duplicados temporales, por `rowId`. Si el frontend no manda exclusiones explicitas, el backend aplica automaticamente las recordadas del perfil; cuando el operador marca `Recordar esta seleccion`, el apply persiste las exclusiones actuales de seccion y producto. En frontend, la tabla global se reemplaza por una UI de secciones expandibles: primero se incluye/excluye la seccion completa y luego, dentro del desplegable, se afinan productos individuales o filas duplicadas.
- Impacto: el operador puede dejar fuera marcas o bloques enteros sin revisar fila por fila, la herramienta recuerda esa decision entre corridas del mismo perfil y los conflictos por duplicados siguen resolviendose sin perder granularidad. La vista principal del preview queda bastante mas simple porque muestra resumen por seccion y oculta el detalle fino hasta que se despliega.
- Alternativas consideradas: seguir recordando exclusiones por `rowId` o persistir solo una bandera booleana por seccion; descartado porque `rowId` cambia entre corridas y una memoria solo por seccion no cubre el caso de excluir productos puntuales dentro de una marca que si se quiere seguir importando.
- Archivos / modulos afectados: `next-stack/apps/api/prisma/{schema.prisma,migrations/20260424143000_add_special_order_profile_remembered_exclusions/migration.sql}`, `next-stack/apps/api/src/modules/catalog-admin/{catalog-admin.controller.ts,catalog-admin-special-order.service.ts,catalog-admin.types.ts}`, `next-stack/apps/web/src/features/catalogAdmin/{AdminSpecialOrderImportPage.tsx,api.ts}`, `project-docs/DECISIONS_LOG.md`, `CHANGELOG_AI.md`.
- Validacion requerida: `db:migrate --workspace @nico/api`, `typecheck --workspace @nico/api`, `test --workspace @nico/api`, `build --workspace @nico/api`, `typecheck --workspace @nico/web`, `test --workspace @nico/web`, `build --workspace @nico/web`, `smoke:web`, `git diff --check`.
- Responsable: Codex + operador humano
...
- Fecha: 2026-04-27
- Estado: aceptada
- Tema: los productos por encargue pueden importar colores desde Google Sheets/CSV, pero el TXT sigue siendo la fuente maestra
- Contexto: el proveedor entrega un TXT con productos y precios USD, y en paralelo una hoja con colores y stock por color. El catalogo no tenia variantes comprables; tratar los colores como texto libre hubiera impedido que el cliente elija color y que el pedido conserve esa decision.
- Decision: agregar `ProductColorVariant` solo para productos `SPECIAL_ORDER`, extender el perfil de importacion con URL opcional de Google Sheet, y permitir que el preview procese una fuente opcional de colores por link publico o CSV. El matching se hace contra productos ya detectados en el TXT del mismo preview; filas sin match quedan como warnings y no crean productos. En tienda, si un producto por encargue tiene colores importados, el cliente debe elegir un color disponible antes de agregarlo al carrito; carrito, checkout y orden guardan el color seleccionado como snapshot.
- Impacto: el flujo de encargues puede publicar variantes de color reales sin convertir todo el catalogo en un sistema general de variantes. La disponibilidad de `SPECIAL_ORDER` con colores pasa a depender de los colores activos, y los pedidos quedan trazables por color elegido sin descontar stock local.
- Alternativas consideradas: guardar colores como descripcion del producto o crear variantes para todo tipo de producto; descartado porque la descripcion no resuelve seleccion comprable y las variantes generales aumentan el alcance sobre inventario real sin necesidad actual.
- Archivos / modulos afectados: `next-stack/apps/api/prisma/{schema.prisma,migrations/20260424190000_add_special_order_color_variants/migration.sql}`, `next-stack/apps/api/src/modules/{catalog-admin,store,cart,orders}`, `next-stack/apps/web/src/features/{catalogAdmin,store,cart,orders}`, `project-docs/{DECISIONS_LOG.md,backend/BACKEND_MAP.md,frontend/FRONTEND_MAP.md,architecture/BUSINESS_RULES.md}`, `CHANGELOG_AI.md`.
- Validacion requerida: `db:migrate --workspace @nico/api`, `typecheck --workspace @nico/api`, `test --workspace @nico/api`, `build --workspace @nico/api`, `typecheck --workspace @nico/web`, `test --workspace @nico/web`, `build --workspace @nico/web`, `smoke:web`, `git diff --check`.
- Responsable: Codex + operador humano
