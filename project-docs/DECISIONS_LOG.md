...
- Decision: mantener `STORE_IMAGE_BASE_URL` solo para assets `storage/` y resolver branding/auth como rutas publicas de la web (`/brand/...`, `/brand-assets/...`, `/icons/...`, favicons). En admin, los slots opcionales sin archivo configurado pasan a mostrarse como vacios y cada card puede declarar `recommendedPx` para informar medidas sugeridas en el mismo punto de carga.
- Impacto: el fondo visual del login y el resto del branding publico usan la misma fuente canonica de assets (`apps/web/public`) sin mezclar host legacy. La UI de identidad visual deja de inducir a error cuando aun no hay imagen cargada y da una referencia concreta de tamanos recomendados para desktop y mobile.
- Alternativas consideradas: seguir prefijando branding con `STORE_IMAGE_BASE_URL` o mostrar previews genericos mientras no haya archivo; descartado porque la primera opcion rompe la fuente canonica de assets y la segunda oculta el estado real de configuracion.
