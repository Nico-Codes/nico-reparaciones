...
- toda carga administrable debe validar extension permitida, tamano maximo y firma/contenido real; los SVG se aceptan solo si no contienen scripts, handlers inline, `javascript:`, `foreignObject` ni referencias externas peligrosas
...
- cuando un asset publico personalizado se reemplaza manteniendo el mismo filename, la URL publica debe agregar `?v=<updatedAt>` para invalidar cache del navegador sin cambiar la ruta canonica del archivo
